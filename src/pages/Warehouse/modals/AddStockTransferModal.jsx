...
function AddStockTransferModal({ row, onClose, onSave, onToast }) {
  ...
  function handleSave() {
    if (!canSave) return;
    if (row) {
      onSave({ ...row, date: ticketDate, carrier, total, note, lines });
      return;
    }
  ...
  return (
    <>
      <SlidePanelShell
        title={row ? "Chi tiết phiếu chuyển kho" : "Thêm phiếu chuyển kho"}
        onClose={onClose}
        tone="brand"
        width={1080}
      >
