...
function AddStockOutModal({ row, onClose, onSave, onToast }) {
  ...
  const [docType, setDocType] = useState(row?.docType ?? STOCK_OUT_DOC_TYPES[0]);
  ...
  function handleSave() {
    if (!canSave) return;
    if (row) {
      onSave({ ...row, date: ticketDate, total, note, target, docType, lines });
      return;
    }
