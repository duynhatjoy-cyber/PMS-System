...
import AddBankVoucherModal from "../modals/AddBankVoucherModal";
import { BANK_VOUCHER_ROWS, BANK_VOUCHER_TYPES } from "../../../data/bankFundData";
...
function BankVoucherPanel({ onToast }) {
  ...
  const [docType, setDocType] = useState(BANK_VOUCHER_TYPES[1]);
  const [rows, setRows] = useState(BANK_VOUCHER_ROWS);
    ...
    <div>
        ...
        <table className={warehouseStyles.table}>
          <thead>
            <tr>
              ...
              <th>
                <span className={warehouseStyles.thLabel}>
                  <DollarSign size={14} /> Số tiền phải trả
                </span>
              </th>
              <th>
                <span className={warehouseStyles.thLabel}>
              ...
              <th className={warehouseStyles.thActionCell}>
                <div className={cashStyles.actionGroup}>
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    className={cashStyles.addBtnChi}
            ...
            <tr className={cashStyles.subHeadRow}>
              <th />
              <th />
              <th />
              <th>VND</th>
            ...
            {pagedRows.length === 0 ? (
              ...
              pagedRows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.account}</td>
                  <td>
                    <button type="button" className={cashStyles.rowLinkGreen}>
                      {row.ticketNo}
                    </button>
                  </td>
      ...
      {printTicket && (
        <PrintPreviewModal
          ...
          fields={[
            { label: "Tài khoản", value: printTicket.account },
